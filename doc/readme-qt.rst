...
Alternatively, install `Qt Creator`_ and open the `icoin-qt.pro` file.
...
- If using HomeBrew,  edit `icoin-qt.pro` to account for library location differences. There's a diff in `contrib/homebrew/icoin-qt.pro.patch` that shows what you need to change, or you can just patch by doing

        patch -p1 < contrib/homebrew/icoin.qt.pro.patch

- Open the icoin-qt.pro file in Qt Creator and build as normal (cmd-B)
...
The static binary version of iCoin is linked against libdb4.8 (see also `this Debian issue`_).
