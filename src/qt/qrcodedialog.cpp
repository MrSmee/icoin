...
{
    QString ret = QString("icoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ICN unit input in lnReqAmount, we generate the URI with ICN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(iCoinUnits::format(iCoinUnits::ICN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
